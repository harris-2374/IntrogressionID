        ...
        for row in df.itertuples():
            ...
            for cattle_record in records:
                ...
                # All cattle should be homozygous for reference
                    ...
                    output_df.drop(index=index, inplace=True)
                    break
                # If not rejected by any criteria, the introgressed position passes and is kept.
                else:
                    break
            continue
        if outputFile:
            after_len = len(output_df)
            logger.info(f"File: {f.name} - Pre-filter total: {before_drop_len:,} - Total positions dropped: {(before_drop_len - after_len):,} - Positions remaining: {(before_drop_len - (before_drop_len - after_len)):,}")
            # Output resulting output_df
